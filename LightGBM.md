...
Uses two cutting-edge techniques (GOSS & EFB) to significantly reduce the time & workload for finding the best split in each base learner.

- **a. Difference between XGboost & LightGBM**  
The most important difference is that GBM needs to go through every possible feature and split points to find the best split. But LightGBM uses GOSS technique to reduce the number of split points that needs checking and uses EFB to reduce the number of features that needs checking, thereby largely increases training efficiency.     
  ...
  |  Only support feature parallel| Support feature parallel/data parallel/Voting parallel  |
...
- **b. what is Histogram Algorithm**  
  ...
  The Histogram Algorithm in LightGBM tries to discretize every feature in the dataset into sets of bins. For example, feature A will be converted into k bins and samples that have A feature value between [0,0.1) fall into bin1, and [0.1,1) fall into bin2, and so on.  
  ...
  - Creating Bins  
    ...
    - For continuous features C  
      ...
      Based on n samples, we can calculate the number of distinct values U in them, and then group samples by these distinct values. We also need to predefine the number of bins we want as B and the maximum number of samples allowed in one bin as M.  

      - If U <= B, then each bin will only contain samples of one unique value. 
      - If U > B, then there will be bins hosting more than one unique value groups. We then count the number of samples in each unique value group.  
      
        For those unique value groups with sample counts more than n/B, each of them will form a distinct bin. 
        
        For the rest of the unique value groups, we will presort them based on the value of feature C then order them from small to large. When the accumulated count reaches M, we finish one bin. So on and forth until all the samples fall into bins.  

    - For Categorical feature C  
      Input: n samples in that parent node.  

      Based on n samples, we can calculate the number of distinct categories U in them, and then we will discard categories with less than 1% of the total samples. Each of the rest categories will fall into a bin (one category inside one bin).
  ...
  - Advantages of using Histogram Algorithm?  

    The [official website of LightGBM](https://github.com/microsoft/LightGBM/blob/master/docs/Features.rst) lists 4 main advantages:  
    - Reduce cost of calculating the gain for each split
      - Presort-based algorithms have time complexity O(#data)  
      - Computing the histogram has time complexity O(#data), but this involves only a fast sum-up operation. Once the histogram is constructed, a histogram-based algorithm has time complexity O(#bins), and #bins is much smaller than #data.  
...
- **c. What is GOSS(Gradient-based One-Side Sampling)**  
  Besides Histogram Algorithm, lightGBM also proposes GOSS method to further reduce RAM usage and to save computing time for finding the best features and best split points.  

  The basic idea behind GOSS is quite similar to Adaboost. Recall that in Adaboost, we keep adjusting the weights for each sample in each iteration. We decrease the weight for correctly identified samples and increase the weight for incorrectly identified samples.  
