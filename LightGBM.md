...
- **b. what is Histogram Algorithm**  
  ...
  This Histogram Algorithm is slightly different from the one used in XGboost. The Histogram Algorithm in LightGBM will try to discretize float number into Ints type and put all the samples into K bins seperated by Int (e.g. (2,3], (3,4],(4,5]). For example, supose x1=2.1, then x1 should fall into bin (2, 3]. Each sample will fall into one of the bins.  
...
- **c. what is GOSS(Gradient-based One-Side Sampling)**  
  ...
  The basic idea behind GOSS is quite similar to Adaboost. Recall 
